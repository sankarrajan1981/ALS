...
interestRec = readRDS("interestRec")
strengthRec = readRDS("strengthRec")
upskillRec  = readRDS("upskillRec")
refreshRec  = readRDS("refreshRec")

interestRecAssesment  = readRDS("interestQuestionsRec")
strengthRecAssesment  = readRDS("strengthQuestionsRec")
upskillRecAssesment   = readRDS("upskillQuestionsRec")
refreshRecAssesment   = readRDS("refreshQuestionsRec")
...
ui <- dashboardPage( 
  ...
  dashboardBody(
    ...
    /* other links in the sidebarmenu when hovered */
                          ...
                          background-color: rgb(252,245,251);color: rgb(0,144,197);font-weight: bold;
                          }                      
...
server <- function(input, output) {
  ...
  userName <- eventReactive(input$login, {
    ...
    print("loaded refresh deck")
    userInterestDeck  <<- as.matrix(interestRec [ interestRec$masked_user_id == input$user,2])
   
    # create challenge deck by picking up randomly from other recommender
    print("before loaded challenge deck")
    challengeRandom  = data.frame()
    
    challengeRandom = rbind( challengeRandom,data.frame(deck_id = userStrengthDeck))
    challengeRandom = rbind( challengeRandom,data.frame(deck_id = userUpkskillDeck))
    challengeRandom = rbind( challengeRandom,data.frame(deck_id = userRefreshDeck))
    challengeRandom = rbind( challengeRandom,data.frame(deck_id = userInterestDeck))
    challengeRandom = challengeRandom %>% group_by(deck_id)%>% count() %>% arrange(desc(n)) %>% head(50)
    challengeRandom$n = NULL
    colnames(challengeRandom) = "deck_id"
    userChallengeDeck <<- as.matrix(challengeRandom)
    print("loaded challenge deck")
    
    # load assesment content for the user
    userStrengthDeckAssesment  <<- as.matrix(strengthRecAssesment [ strengthRecAssesment$masked_user_id == input$user,2])
    userUpkskillDeckAssesment  <<- as.matrix(upskillRecAssesment  [ upskillRecAssesment$masked_user_id == input$user,2])
    userRefreshDeckAssesment  <<- as.matrix(refreshRecAssesment [ refreshRecAssesment$masked_user_id == input$user,2])
    userInterestDeckAssesment  <<- as.matrix(interestRecAssesment [ interestRecAssesment$masked_user_id == input$user,2])
    
    # create challenge DeckAssesment by picking up RandomAssesmently from other recommender
    
    challengeRandomAssesment  = data.frame()
    challengeRandomAssesment = rbind( challengeRandomAssesment,data.frame(deck_id = userStrengthDeckAssesment))
    challengeRandomAssesment = rbind( challengeRandomAssesment,data.frame(deck_id = userUpkskillDeckAssesment))
    challengeRandomAssesment = rbind( challengeRandomAssesment,data.frame(deck_id = userRefreshDeckAssesment))
    challengeRandomAssesment = rbind( challengeRandomAssesment,data.frame(deck_id = userInterestDeckAssesment))
    challengeRandomAssesment = challengeRandomAssesment %>% group_by(deck_id)%>% count() %>% arrange(desc(n)) %>% head(50)
    challengeRandomAssesment$n = NULL
    userChallengeDeckAssesment <<- as.matrix(challengeRandomAssesment)
